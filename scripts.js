  ...
  tableItem.forEach((item) => {
    const readButtonText =
      item.firstElementChild.nextElementSibling.nextElementSibling
        .firstElementChild.textContent; // Retrieve text content of read buttons
    if (filter === "all") {
...
submit.addEventListener("click", () => {
  if (!bookField.checkValidity() || !authorField.checkValidity()) return;
  addBookToLibrary();
  bookField.value = "";
  authorField.value = "";
...
document.addEventListener("click", (event) => {
  if (
    ...
    !event.target.classList.contains("new-book-button") &&
